...
# -----------------------------------------------------------------------------
# repair: reduz PR depois J e X ate total_units <= LIMIT_UNITS
# nota: alteracoes dentro de funcao sao locais em R — a solucao reparada
# e devolvida explicitamente (nao via <<-)
# -----------------------------------------------------------------------------
repair=function(S)
{ S=normalize(S)
iter_max=500; it=0
while(total_units(S)>LIMIT_UNITS && it<iter_max)
{ # primeiro reduz promocoes
  S[idx_PR]=S[idx_PR]*0.95
  S[idx_PR]=pmin(S[idx_PR],upper[idx_PR])
  S[idx_PR]=pmax(S[idx_PR],lower[idx_PR])
  # se ainda nao chega, reduz funcionarios
  if(total_units(S)>LIMIT_UNITS)
  { S[idx_J]=S[idx_J]*0.95
  S[idx_X]=S[idx_X]*0.95
  }
  S=normalize(S)
  it=it+1
}
...
# Death Penalty: descarta solucoes invalidas devolvendo um valor muito mau
# o PSO ainda consegue navegar porque as solucoes validas dao valores finitos
eval_death=function(S)
{ S=normalize(S)
EV<<-EV+1
if(total_units(S)>LIMIT_UNITS)
{ if(EV<=MAXIT) F[EV]<<-BEST
return(1e9)  # valor muito mau mas finito — melhor que Inf para o PSO
}
res=profit(S)
...
{ cat("\n=== PSO O2 -",nome,"(",RUNS,"runs) ===\n")
  ...
  { cat("run",run,"/",RUNS,"...\n")
    ...
    BEST<<- -Inf
    MAXIT<<-MAXIT_local
    F<<-rep(NA,MAXIT_local)
    
    set.seed(run)
    s0=lower+(upper-lower)*runif(D)
    
    ps=psoptim(par=s0,fn=fn_eval,lower=lower,upper=upper,
  ...
  return(list(lucros=lucros,F_mat=F_mat,
              S_best=S_best_global,lucro_best=lucro_best_global))
}
...
av=1:length(F_clean)
pdf(file.path(output_dir,paste0("convergencia_PSO_O2_",nome,".pdf")),width=10,height=6)
...
# CSV convergencia
write.csv(data.frame(Avaliacao=av,Lucro_Mediana=round(F_clean,2)),
...
ymin=min(c(F_d,F_r),na.rm=TRUE)
ymax=max(c(F_d,F_r),na.rm=TRUE)

pdf(file.path(output_dir,"comparacao_PSO_O2.pdf"),width=10,height=6)
plot(1:length(F_d),F_d,type="l",lwd=2,col="blue",
     ylim=c(ymin,ymax),
     main=paste("PSO O2 — DeathPenalty vs Repair (runs=",RUNS,")"),
     xlab="Numero de avaliacoes da funcao profit()",
     ylab="Mediana do melhor lucro acumulado ($)")
lines(1:length(F_r),F_r,lwd=2,col="darkgreen")
...
tabela=data.frame(
  ...
  Unidades_Best=round(c(total_units(res_death$S_best),
                        total_units(res_repair$S_best)),2),
  Tempo_Total_s=round(tempo_total,2)
)

write.csv(tabela,file=file.path(output_dir,"tabela_resumo_PSO_O2.csv"),row.names=FALSE)
...
write.csv(data.frame(Run=1:RUNS,
          ...
          file=file.path(output_dir,"lucros_runs_PSO_O2.csv"),row.names=FALSE)

...
# Mesmo padrão que SANN v2, HC e Tabu
# =============================================================
repair <- function(S) {
...
# =============================================================
# MONITOR FES — mesmo padrão que OTIM_GA_v2.R (Joao)
...
# eval_func: recebe bits, retorna valor a minimizar
# label_O2: se TRUE, verifica restricao total_units <= 10000
# =============================================================
run_gabin <- function(eval_func, label, seed_base, cor_med = "steelblue",
                      pdf_name, rds_name, label_O2 = FALSE) {
  cat(sprintf("\n=== rbga.bin %s (%d runs) ===\n", label, NRUNS))
  EVAL_FUN <<- eval_func
  ...
  for (r in 1:NRUNS) {
    ...
    # Para O2 Repair: extrair a solução reparada (a eval já reparou internamente,
    # mas S_best vem do decode sem repair — recalculamos)
    if (grepl("Repair", label)) {
      S_rep <- repair(S_best)
      S_best <- if (!is.null(S_rep)) S_rep else S_best
    }
...
# =============================================================
eval_bin_O1 <- function(bits) { -profit(decode_S(bits)) }
...
# =============================================================
# O2 — Death Penalty
# Soluções com total_units > 10000 recebem penalidade 1e9
# =============================================================
eval_bin_O2_dp <- function(bits) {
  S <- decode_S(bits)
  u <- total_units(S)
...
res_O2_dp <- run_gabin(
  eval_func = eval_bin_O2_dp,
  label     = "O2 (Death Penalty)",
  seed_base = 123,
  cor_med   = "tomato",
  pdf_name  = "convergencia_O2_dp_gabin.pdf",
  rds_name  = "convergencia_O2_dp_gabin.rds",
  label_O2  = TRUE
)
...
# Soluções irreparáveis recebem penalidade 1e9
# =============================================================
eval_bin_O2_rep <- function(bits) {
  S   <- decode_S(bits)
  S_r <- repair(S)
...
res_O2_rep <- run_gabin(
  eval_func = eval_bin_O2_rep,
  label     = "O2 (Repair)",
  seed_base = 456,
  cor_med   = "darkorange",
  pdf_name  = "convergencia_O2_rep_gabin.pdf",
  rds_name  = "convergencia_O2_rep_gabin.rds",
  label_O2  = TRUE
)

# RDS principal O2 = Repair (para comparativos.R)
saveRDS(res_O2_rep$med_curve, file.path(GABIN_DIR, "convergencia_O2_gabin.rds"))
...
curve_dp  <- res_O2_dp$med_curve
curve_rep <- res_O2_rep$med_curve

pdf(file.path(GABIN_DIR, "convergencia_O2_dp_vs_repair_gabin.pdf"), width = 10, height = 6)
...
if (length(all_vals) > 0) {
  ...
  plot(NA, xlim = c(1, x_max), ylim = range(all_vals),
       ...
       main = paste0("rbga.bin O2 — Death Penalty vs Repair (", NRUNS, " runs)"))
  if (!is.null(curve_dp))

# =============================================================
# fase_integrada_v3.R — Eduardo
# Integração Forecasting → Otimização — VERSÃO FINAL 12 SEMANAS
#
...
LOG_FILE  <- file.path(OUT_DIR, "log_fase_integrada_v3.txt")
...
setwd(file.path(BASE_DIR, "data", "tratados"))
baltimore    <- read.csv("baltimore_tratado.csv",    stringsAsFactors = FALSE)
lancaster    <- read.csv("lancaster_tratado.csv",    stringsAsFactors = FALSE)
richmond     <- read.csv("richmond_tratado.csv",     stringsAsFactors = FALSE)
philadelphia <- read.csv("philadelphia_tratado.csv", stringsAsFactors = FALSE)
...
RUNS  <- 12    # iteracoes de backtesting (versao final)
W     <- 630   # janela inicial: 714 - H - (RUNS-1)*S = 714 - 7 - 77 = 630

# Parâmetros dos algoritmos (versao final)
NRUNS_BT    <- 5     # runs por semana
N_ITER_MC   <- 5000  # MC: avaliacoes por run
MAX_ITER_HC <- 5000  # HC: iteracoes por run
MAXIT_SANN  <- 3000  # SANN: iteracoes
...
NGENS_NS    <- 200   # NSGA-II: geracoes
NRUNS_NS    <- 3     # NSGA-II: runs por semana
...
forecast_rf_c2 <- function(dados, b) {
  ...
  L    <- length(nc)
  ND   <- L - MAX_LAG_RF
  W_rf <- (ND - H) - (RUNS - 1) * S  # = 602 para RUNS=12
...
# =============================================================
cat("\n=== FASE INTEGRADA — 12 SEMANAS (VERSÃO FINAL) ===\n")
...
# =============================================================
# SECÇÃO 5 — EXPORTAR (ficheiros v3 — não sobrescreve v2)
# =============================================================

write.csv(tabela_final,
          file.path(OUT_DIR, "tabela_comparativa_final_v3.csv"),
          row.names = FALSE)
...
write.csv(df_mat,
          file.path(OUT_DIR, "profits_por_semana_v3.csv"),
          row.names = FALSE)
...
write.csv(df_prev[, c("semana", colnames(df_prev)[-29])],
          file.path(OUT_DIR, "prev_12_semanas_v3.csv"),
          row.names = FALSE)
...
pdf(file.path(OUT_DIR, "g1_boxplot_O1_O2_v3.pdf"), width = 12, height = 6)
...
pdf(file.path(OUT_DIR, "g2_lucro_por_semana_O1_v3.pdf"), width = 11, height = 5)
...
pdf(file.path(OUT_DIR, "g3_lucro_por_semana_O2_v3.pdf"), width = 11, height = 5)
...
pdf(file.path(OUT_DIR, "g4_barplot_medianas_v3.pdf"), width = 11, height = 6)
par(mar = c(5, 5, 3, 1))
...
pdf(file.path(OUT_DIR, "g5_nsga2_O3_por_semana_v3.pdf"), width = 9, height = 5)
par(mar = c(4, 4.5, 3, 1))
...
pdf(file.path(OUT_DIR, "g6_heatmap_profits_v3.pdf"), width = 10, height = 6)
par(mar = c(5, 6, 3, 5))
...
pdf(file.path(OUT_DIR, "g7_prev_por_semana_v3.pdf"), width = 10, height = 5)
par(mar = c(4, 4.5, 3, 1))
...
saveRDS(
  ...
  file.path(OUT_DIR, "resultado_fase_integrada_v3.rds")
)
...
# SUMÁRIO FINAL
# =============================================================
cat("\n=== SUMÁRIO FASE INTEGRADA — 12 SEMANAS ===\n")

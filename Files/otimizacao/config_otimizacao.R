...
# -------------------------------------------------------------
PREV <- c(
  114, 114, 108, 110, 85, 97, 164,   # Baltimore
  133, 115, 112, 111, 109, 112, 142, # Lancaster
  232, 167, 180, 190, 196, 209, 302, # Philadelphia
  64, 45, 47, 49, 52, 59, 82         # Richmond
)
...
calc_upper <- function(PREV) {
  ...
  for (i in 1:4) {
    for (d in 1:7) {
      ...
      upper <- c(upper, 0.30, ceiling(C / 6), ceiling(C / 7))
    }
  }
  return(upper)
}
...
# 5. FUNÇÃO EVAL
# -------------------------------------------------------------
eval <- function(S) return(-profit(S))
...
# -------------------------------------------------------------
# VALIDAÇÃO — todas as funções já definidas
# -------------------------------------------------------------
